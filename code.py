...
def generate_with_llama(prompt):
    try:
        response = ollama.generate(model="llama3.2", prompt=f"{prompt} Provide only the concise, complete text or numbered list (no introductory phrases, no formatting). Ensure 5 to 7 complete bullet points ending with full sentences, derived solely from the provided CSV data analysis.")
        return response['response'].strip()
    except Exception:
        return "Analysis failed due to error.\nPlease verify CSV data integrity.\nContact support if issue persists.\nData analysis could not proceed.\nThis is an error message."

# Function to split text into 5 to 7 bullet points, splitting if >7
def split_into_bullets(text, min_points=5, max_points=7):
...
def generate_eda_report(csv_file, col, plot_type, min_slides, user_prompt):
    ...
    max_points_per_slide = 7  # Limit to 7 to fit, split if more
    ...
    # Check if extra slides are needed
    extra_slides_needed = min_slides > (2 * num_cols)
    
    # Add introduction slide (always after overview)
    intro_prompt = f"Introduce analysis of {col} vs others based on CSV with {len(df)} rows, {num_cols} columns, focusing on {col} in 5 to 7 bullet points based on '{user_prompt}'."
    intro_text = generate_with_llama(intro_prompt)
    intro_points = split_into_bullets(intro_text)
    if len(intro_points) > 7:
        add_slide(prs, "Introduction to Analysis", intro_points[:7])
        add_slide(prs, "Introduction to Analysis Continued", intro_points[7:])
    else:
    ...
    for idx, other_col in enumerate(other_cols):
        ...
        content_points = [
            ...
            f"Plot type: {actual_plot_type}. Visualizes {col} vs {other_col}."
        ]
        add_slide(prs, f"Comparison Insights: {col} vs {other_col}", content_points)
        
        # Detailed insights slide (CSV-specific)
        detail_prompt = f"Provide detailed insights for {col} vs {other_col} based on CSV data: '{stats_content}', in 5 to 7 bullet points based on '{user_prompt}'."
        detail_text = generate_with_llama(detail_prompt)
        detail_points = split_into_bullets(detail_text)
        if len(detail_points) > 7:
            add_slide(prs, f"Detailed Insights: {col} vs {other_col}", detail_points[:7])
            add_slide(prs, f"Detailed Insights: {col} vs {other_col} Continued", detail_points[7:])
            slide_titles.append(f"Detailed Insights: {col} vs {other_col} Continued")
        else:
    ...
    if extra_slides_needed:
        ...
        if len(index_points) > 7:
            add_slide(prs, "Index of Slides", index_points[:7])
            add_slide(prs, "Index of Slides Continued", index_points[7:])
            slide_titles.append("Index of Slides Continued")
        else:
    ...
    if user_prompt and user_prompt.lower() != "default analysis of one column vs others" and "summary" in user_prompt.lower():
        summary_prompt = f"Summarize analysis of {col} vs others based on CSV data with {len(df)} rows, {num_cols} columns in 5 to 7 bullet points based on '{user_prompt}'."
        summary_text = generate_with_llama(summary_prompt)
        summary_points = split_into_bullets(summary_text)
        if len(summary_points) > 7:
            add_slide(prs, "Summary of Findings", summary_points[:7])
            add_slide(prs, "Summary of Findings Continued", summary_points[7:])
            slide_titles.append("Summary of Findings Continued")
        else:
    ...
    if current_slides < min_slides:
        for i in range(min_slides - current_slides):
            extra_prompt = f"Provide extra analysis for {col} vs others based on CSV data with {len(df)} rows, {num_cols} columns in 5 to 7 bullet points based on '{user_prompt}'."
            extra_text = generate_with_llama(extra_prompt)
            extra_points = split_into_bullets(extra_text)
            if len(extra_points) > 7:
                add_slide(prs, f"Additional Analysis {i + 1}", extra_points[:7])
                add_slide(prs, f"Additional Analysis {i + 1} Continued", extra_points[7:])
                slide_titles.append(f"Additional Analysis {i + 1} Continued")
            else:
    ...
    conclusion_prompt = f"Conclude analysis of {col} vs others based on CSV data with {len(df)} rows, {num_cols} columns in 5 to 7 bullet points based on '{user_prompt}'."
    conclusion_text = generate_with_llama(conclusion_prompt)
    conclusion_points = split_into_bullets(conclusion_text)
    if len(conclusion_points) > 7:
        add_slide(prs, "Conclusion of Analysis", conclusion_points[:7])
        add_slide(prs, "Conclusion of Analysis Continued", conclusion_points[7:])
        slide_titles.append("Conclusion of Analysis Continued")
    else:
